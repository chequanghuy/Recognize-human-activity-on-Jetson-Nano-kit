...
class YoLov5TRT(object):
    ...
    def infer(self, image_raw):
        ...
        return image_raw,result_boxes, end - start
    def infer_batch(self, raw_image_generator):
        threading.Thread.__init__(self)
        # Make self the active context, pushing it on top of the context stack.
        self.ctx.push()
        # Restore
...
#     def run(self):
#         # print("abc",self.image_raw.shape)
        
#         # for i, img_path in enumerate(self.image_path_batch):
#         #     parent, filename = os.path.split(img_path)
#         #     save_name = os.path.join('output', filename)
#         #     # Save image
#         # cv2.imwrite("cap.jpg",image_raw)
#         while cap.isOpened():
#             batch_imgs=[]
...
if __name__ == "__main__":
    ...
    try:
        cap = cv2.VideoCapture("/home/huycq/Downloads/video.mp4")
        cap.set(3,640)
        ret, frame = cap.read()
        cap.set(4,480)
